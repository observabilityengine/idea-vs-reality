...
export default function App() {
  ...
  const [completed, setCompleted] = useState<Memory[]>([]);
  ...
  const refresh = useCallback(async () => {
    try {
      const [active, done] = await Promise.all([listActiveMemories(), listCompletedMemories()]);
      setMemories(orderMemories(active));
      setCompleted(done);
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to load memories.'); }
  }, []);
  ...
  const addMemory = async (input: string) => {
    ...
    if (!validated) { setError('Memory must contain text and be 500 characters or fewer.'); return; }
    await createMemory(validated); await refresh();
  };
  ...
  return (
    <GestureHandlerRootView style={styles.root}>
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}><Text style={styles.title}>Head Check</Text><Pressable onPress={() => setHistory(v => !v)} accessibilityRole="button"><Text style={styles.history}>{history ? 'Active' : `History${completed.length ? ` (${completed.length})` : ''}`}</Text></Pressable></View>
        {error && <View accessibilityRole="alert" style={styles.errorBanner}><Text style={styles.errorText}>{error}</Text></View>}
        {history ? <MemoryList memories={completed} onComplete={() => undefined} /> : <MemoryList memories={memories} onComplete={id => void finishMemory(id)} />}
        {!history && <AddMemory onSubmit={addMemory} onError={setError} />}
        {!history && undoId && <View style={styles.undo}><Text style={styles.undoText}>Completed</Text><Pressable onPress={() => void undo()} accessibilityRole="button"><Text style={styles.undoAction}>Undo</Text></Pressable></View>}
      </SafeAreaView>
    </GestureHandlerRootView>
  );
}
const styles = StyleSheet.create({ root: { flex: 1 }, safeArea: { flex: 1, backgroundColor: theme.colors.background }, header: { paddingHorizontal: theme.spacing.page, paddingTop: 12, paddingBottom: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, title: { color: theme.colors.text, fontSize: 30, fontWeight: '600' }, history: { color: theme.colors.secondaryText, fontSize: 16 }, content: { flex: 1 }, errorBanner: { marginHorizontal: theme.spacing.page, marginBottom: 4, padding: 10, borderRadius: 10, backgroundColor: theme.colors.inputBackground }, errorText: { color: theme.colors.text, fontSize: 15 }, undo: { marginHorizontal: theme.spacing.page, marginBottom: 8, padding: 12, borderRadius: 12, backgroundColor: theme.colors.inputBackground, flexDirection: 'row', justifyContent: 'space-between' }, undoText: { color: theme.colors.secondaryText }, undoAction: { color: theme.colors.text, fontWeight: '700' } });
